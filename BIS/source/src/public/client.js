...
;(()=>{
    ...
    // @ts-ignore
    const inputEl = document.getElementById('chatBox')

    if(!formEl || !inputEl || !chatListEl){
    ...
    //const myNickColor = `#${Math.floor(Math.random() * 127 + 128). toString(16)}${Math.floor(Math.random() * 127 + 128). toString(16)}${Math.floor(Math.random() * 127 + 128). toString(16)}`
